...
//para borrar ya no es necesario tomar datos en el array ya que podemos hacer uso de la sesion
...
//se llama a la funcion delete especificando el usuario a borrar(el correo)
delete("correo",$_SESSION["usuario"]);
...
function delete($clave,$valor){
    ...
    switch ($clave) {
        ...
        case "ci":
            ...
            */
        case "correo":
            //se borran dartos de la tabla cuenta
            $sql="DELETE FROM cuenta WHERE correo='$valor';";
            if (!$mysqli -> query($sql)) {                                
                //echo("<br>ERROR EN EL BORRADO DE CORREO: " . $mysqli -> error);   
                //echo "<BR> consulta original: ".$sql;
                error("<br>ERROR EN EL BORRADO DE CORREO EN LA TABLA CUENTA: " . $mysqli -> error);
            }

            //se borran dartos de la tabla password
            $sql="DELETE FROM password WHERE usuario='$valor';";
            if (!$mysqli -> query($sql)) {                                
                //echo("<br>ERROR EN EL BORRADO DE CORREO: " . $mysqli -> error);   
                //echo "<BR> consulta original: ".$sql;
                error("<br>ERROR EN EL BORRADO DE CORREO EN LA TABLA CUENTA: " . $mysqli -> error);
            ...
            allOk("Cuenta eliminada"); //mostramos un mensaje
            goUrl("../index.php");      //regresamos al inicio
    }
}
